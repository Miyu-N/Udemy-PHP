...
if (isset($_GET['action']) && $_GET['action'] === 'rewrite' && isset($_SESSION['form'])) {//書き直しの場合
    $form = $_SESSION['form'];
} else {
    $form = [
        'name' => '',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    } else if (strlen($form['password']) < 4) {//strlen: 文字列の長さを得る
    ...
    if ($image['name'] !== '' && $image['error'] === 0) {
        $type = mime_content_type($image['tmp_name']);//ファイルのタイプを検出
        if ($type !== 'image/png' && $type !== 'image/jpeg') {
            $error['image'] = 'type';
        }
    }

    if (empty($error)) {//エラーが空だったら＝エラーがなければ
        $_SESSION['form'] = $form;
